...
競輪の三連単で**10,000円以上の高配当を予測する機械学習プロジェクト**です。LightGBMと時系列クロスバリデーションを使用しています。

> **注意**: 以前報告されていた高精度（ROC-AUC 0.841、Precision@Top100 = 1.0）は、
> `trifecta_popularity`（三連単人気順位）という**事後データ**を特徴量に含めていたことによる
> データリーケージが原因でした。この特徴量はレース確定後にしか得られないため、
> 予測には使用できません。現在は除外済みで、モデルの再訓練が必要です。
...
**特徴量**（事前データのみ使用）:
- `heikinTokuten_cv` - 選手平均得点の変動係数
- `category` - レースカテゴリ（S級、A級など）
- `track` - トラックの種類/会場
- `nigeCnt_cv`, `makuriCnt_cv` など - 脚質のばらつき
- `entry_count`, `narabi_flg` など - レース構成情報

> **除外済み**: `trifecta_popularity`（事後データのためリーケージ）
...
- **事前オッズデータなし**: 投票締切前のオッズ情報があれば精度向上が期待できる（事後の人気順位はリーケージのため使用不可）
