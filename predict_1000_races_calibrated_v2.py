...
# -*- coding: utf-8 -*-
"""
1,000レース予測 - キャリブレーション統合版
選手詳細データから特徴量を計算し、キャリブレーション済みの予測を実行
"""
...
# キャリブレーション係数（分析により決定）
CALIBRATION_SCALE = 0.7151  # 比率スケーリング: 0.3413 / 0.4773

print("="*80)
print("【1,000レース 選手詳細データで予測実行（キャリブレーション版）】")
print("="*80)
...
def calculate_features_from_riders(race_data):
    ...
    style_ryo_count = style_counts.get('差', 0)
...
print("\n" + "="*80)
print("【1,000レース 予測結果（キャリブレーション版）】")
print("="*80)
...
# 保存
results_df.to_csv('analysis/model_outputs/1000_races_predictions_calibrated_v2.csv', index=False)
print(f"\n✓ 保存完了: analysis/model_outputs/1000_races_predictions_calibrated_v2.csv")
...
summary = {
    ...
    'prediction_method': 'Rule-based with collected rider data + calibration (scale=0.7151)',
    'calibration_scale': CALIBRATION_SCALE,
    'threshold_results': results_table.to_dict('records')
}

with open('analysis/model_outputs/1000_races_summary_calibrated_v2.json', 'w', encoding='utf-8') as f:
    json.dump(summary, f, ensure_ascii=False, indent=2)

print(f"✓ 保存完了: analysis/model_outputs/1000_races_summary_calibrated_v2.json")
...
print(f"  - 1,000レース（簡易推定）: 71.90%")
print(f"  - 1,000レース（詳細推定・元）: 64.10%")
print(f"  - 1,000レース（詳細推定・キャリブレーション版）: {best_accuracy*100:.2f}%")
print("="*80)
